...
class ApplyRequiredRoleContextTests(unittest.TestCase):
    def test_validate_warns_when_role_context_missing(self):
        ...
        self.assertIn("role_context_missing", result["role_context_warnings"])

    def test_dry_run_warns_when_recommended_field_missing(self):
        capsule_data = build_capsule()
        ...
        self.assertEqual(result["status"], "ok")
        self.assertIn("role_context_missing_recommended_field:symmetry_basis", result["role_context_warnings"])

    def test_apply_blocks_when_role_context_missing(self):
        capsule_data = build_capsule()
        capsule_data.pop("role_context")
        capsule, receipt, authority = bound_objects(capsule_data)
    ...
    def test_apply_blocks_when_recommended_field_missing(self):
        capsule_data = build_capsule()
        capsule_data["role_context"].pop("symmetry_basis")
        capsule, receipt, authority = bound_objects(capsule_data)
    ...
    def test_apply_allows_complete_role_context(self):
